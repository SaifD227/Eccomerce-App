...
const BagsPage = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get<Product[]>(
          "https://fakestoreapi.com/products/category/accessories" // Fetching from accessories category
        );
  ...
  return (
    <div>
      <Navbar />
      <h1 className="text-2xl font-bold mt-4">Bags Items</h1>
      <ul className="mt-4 space-y-4">
        {bags.length > 0 ? (
          ...
          <li className="text-gray-500">No bags found.</li>
        )}
      </ul>
    </div>
  );
